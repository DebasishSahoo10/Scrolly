...
export const NewPostField = ({ sortFunc }) => {
  ...
  const handleFileChange = (e) => {
    ...
    setInput((prev) => ({ ...prev, img: URL.createObjectURL(file) }));
  };
  useEffect(() => {
    if (data.editingPost) {
      setInput((prev) => ({ ...prev, content: data.editableContent }));
    }
  ...
  return (
    <div className={PostFieldStyles.newField}>
      ...
      <div
        ...
        className={PostFieldStyles.imageInput}
      >
        <button>
          {input.postData.img ? "Image Selected 📸" : "Select Image 📷"}
        </button>
        <input
          type="file"
          ref={inputRef}
      ...
      <button
        onClick={() =>
          handleUpload(input, toast, data, auth, dispatch, sortFunc)
        }
      >
        Upload ✅
      </button>
      <button onClick={() => dispatch(POSTFIELD_FALSE())}>Dismiss 🕯️</button>
    </div>
  );
};
